...
async def seed(conn: asyncpg.Connection) -> None:
    ...
    for resume_slug, variant in variants.items():
        for cat_order, cat in enumerate(variant.get('skills', [])):
            ...
            # Delete old skills for this category and re-insert
            # (simpler than tracking individual skill changes)
            await conn.execute(
...
async def _upsert_highlights(
    ...
    # Delete existing highlights for this role+resume combo, then re-insert
    await conn.execute(
